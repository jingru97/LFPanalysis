...
userfol='/Volumes/User/jingru';
%savefolder='/seseyev6_1/';
date_folder={'20171213'};

for date = date_folder
...
function [mrawspec,mspec,mvlspec] = combfun(navsession,channel,array,session,date,userfol)

% Initialises directory name
folder=strcat('/Volumes/Hippocampus/Data/picasso-old/',date,'/',session,'/',array,'/',channel);

session_folder=strcat('/Volumes/Hippocampus/Data/picasso-old/',date,'/', navsession, '/',array,'/',channel);
...
if exist(char(session_folder), 'dir')==7 % Check that the folder exists
    ...
    cd(char(strcat(userfol,'/Desktop/results')))
    mkdir(char(strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,'/',navsession,'/',channel)))
...
%     saveas(gcf,strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,savefolder,channel,'/signalaverage_',date,'_',channel,'.png'))
%     close
%     
    % ===============================================================
    ...
    saveas(gcf,char(strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,'/',navsession,'/',channel,'/PSD_rawspec_',date,'_',channel,'.png')))
    close
    
    % compute and save normalised spectrogram plot
    plotspec(mspec,t1,t2,1,50,c1,c2);
    saveas(gcf,char(strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,'/',navsession,'/',channel,'/norm_-500to-200_PSD_',date,'_',channel,'.png')))
    close
    ...
    saveas(gcf,char(strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,'/',navsession,'/',channel,'/norm_-500to-200_PS_',date,'_',channel,'.png')))
    close
    ...
    cd(char(strcat('/Volumes/Hippocampus/Data/picasso-old/',date,'/',navsession,'/',array,'/',channel)))
    vl = vmlfp('auto');
    ...
    saveas(gcf,char(strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,'/',navsession,'/',channel,'/PSD_vlnormspec_',date,'_',channel,'.png')))
    close
    
    %power spectrum
    localpowerspectrum(mvlspec,1,50,c1,c2);
    saveas(gcf,char(strcat(userfol,'/Desktop/results/Hippocampal LFP Research/',date,'/',navsession,'/',channel,'/norm_seseye_PS_',date,'_',channel,'.png')))
    close
...
Pnorm3=mean(mspec.Pnorm(:,26:36),2); % Movement-onset
